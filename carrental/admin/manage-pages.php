...
if($_POST['submit']=="Update")
{
    ...
    if($pagetype == 'aboutus') {
        ...
        $sql = "UPDATE tblpages SET detail=:pagedetails, experience_years=:experience";
        
        if(!empty($_FILES['image1']['name'])) {
            $image1 = $_FILES['image1']['name'];
            move_uploaded_file($_FILES["image1"]["tmp_name"], "img/vehicleimages/".$image1);
            $sql .= ", image1=:image1";
        }
        if(!empty($_FILES['image2']['name'])) {
            $image2 = $_FILES['image2']['name'];
            move_uploaded_file($_FILES["image2"]["tmp_name"], "img/vehicleimages/".$image2);
            $sql .= ", image2=:image2";
        }
        
        $sql .= " WHERE type=:pagetype";
...
<head>
	...
	<title>Torq | Admin Manage Pages</title>
...
<script type="text/javascript">
	bkLib.onDomLoaded(function() { nicEditors.allTextAreas() });
			...
			?>
									...
									<?php if($_GET['type']=='aboutus') { 
                                        $pagetype=$_GET['type'];
                                        $sql = "SELECT image1, image2, experience_years from tblpages where type=:pagetype";
                                        $query = $dbh -> prepare($sql);
                                        $query->bindParam(':pagetype',$pagetype,PDO::PARAM_STR);
                                        $query->execute();
                                        $resultStats=$query->fetch(PDO::FETCH_OBJ);
                                    ?>
                                    <div class="form-group">
                                        <label class="col-sm-4 control-label">Experience Years</label>
                                        <div class="col-sm-8">
                                            <input type="number" class="form-control" name="experience" value="<?php echo htmlentities($resultStats->experience_years ? $resultStats->experience_years : 10); ?>" required>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label class="col-sm-4 control-label">Vertical Image (Main)</label>
                                        <div class="col-sm-8">
                                            <input type="file" name="image1">
                                            <?php if($resultStats->image1) { ?><br><img src="img/vehicleimages/<?php echo htmlentities($resultStats->image1);?>" width="150" style="margin-top:10px; border-radius:5px;"><?php } ?>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label class="col-sm-4 control-label">Horizontal Image (Secondary)</label>
                                        <div class="col-sm-8">
                                            <input type="file" name="image2">
                                            <?php if($resultStats->image2) { ?><br><img src="img/vehicleimages/<?php echo htmlentities($resultStats->image2);?>" width="150" style="margin-top:10px; border-radius:5px;"><?php } ?>
                                        </div>
                                    </div>
                                    <div class="hr-dashed"></div>
                                    <?php } ?>
